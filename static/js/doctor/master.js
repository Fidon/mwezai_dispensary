...
formData.append("depart_id", 4);
$.ajax({
    type: 'POST',
    url: "/doc/count-patients/",
    data: formData,
    dataType: 'json',
    contentType: false,
    processData: false,
    ...
    success: function(response) {
        if(response.success) {
            ...
            var res = response.res_count;
            $("#patCount").html("<i class='fas fa-hospital-user'></i> &nbsp; Patients ("+count+")");
            $("#results_count").html("<i class='fas fa-file-alt'></i> &nbsp; Results ("+res+")");
        }
    },
