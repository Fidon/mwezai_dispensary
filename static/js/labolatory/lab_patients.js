...
$("#testsListTable tbody tr").click(function (e) { 
    e.preventDefault();
    if ($(this).attr('class') !== "blank_row") {
        $("#test_num").val($(this).attr('id'));
        $("#h6").text($("td.testName", this).text());
        $("#testResultForm").modal({ backdrop: 'static', keyboard: false });
        $("#testResultForm").modal('show');
    }
});


// Add row with input fields
$("#row_btn button").click(function (e) { 
    e.preventDefault();
    var newRow = `<tr>
    <td> <input type="text" class="field1 form-control text-ttxt1" placeholder="Item name"/></td>
    <td><input type="text" class="field2 form-control text-ttxt1" placeholder="Value"/></td>
    <td><input type="text" class="field3 form-control text-ttxt1" placeholder="Normal range"/></td>
    </tr>`;
    $('#row_btn').before(newRow);
});
...
$("#testResultForm").submit(function (e) { 
    e.preventDefault();
    var test_micro = $.trim($("#test_micro").val());
    var test_macro = $.trim($("#test_macro").val());
    var test_report = $.trim($("#test_report").val());
    var micro_macro = (test_micro && !test_macro) || (!test_micro && test_macro);
    if (!micro_macro) {
        var micro = (test_micro.length > 0) ? test_micro : "-";
        var macro = (test_macro.length > 0) ? test_macro : "-";
        var report = (test_report.length > 0) ? test_report : "-";
        FIELDS.push($("#test_num").val(), micro, macro, report);
        $("#testResultForm table tbody tr").each(function () {
            if($(this).attr('id') !== "row_btn") {
                var fd1 = $.trim($("input.field1", this).val());
                var fd2 = $.trim($("input.field2", this).val());
                var fd3 = $.trim($("input.field3", this).val());
                if((fd1.length > 0) && (fd2.length > 0) && (fd3.length > 0)) {
                    var rowInput = {
                        itemName: fd1,
                        itemValue: fd2,
                        itemRange: fd3
        ...
        $.ajax({
            type: 'POST',
            url: "/labolatory/testresult/submit/",
            data: json_data,
            dataType: 'json',
            ...
            beforeSend: function() {
                $("#testSubmitBtn").html("<i class='fas fa-spinner fa-pulse'></i>");
                $("#testSubmitBtn").attr('type','button');
                $("#cancel_btn").fadeOut('fast')
            },
            success: function(response) {
                $("#testSubmitBtn").html("Submit");
                $("#testSubmitBtn").attr('type','submit');
                $("#cancel_btn").fadeIn('fast')
                $('html, body').animate({ scrollTop: 0 }, 'slow');
                if(response.success) {
                    $("#testResultForm").modal('hide');
                    FIELDS.splice(0, FIELDS.length);
                    $("#testResultForm")[0].reset();
                    var tr_id = $("#test_num").val();
                    $("#"+tr_id).slideUp('slow');
                    ...
                    $("#testFormSms").html("<span class='alert alert-danger d-block w-100 m-0 mb-2 p-2'><i class='fas fa-exclamation-circle'></i> "+response.sms+"</span>").slideDown('fast').delay(4000).slideUp('fast');
                }
            },
        ...
        $("#testFormSms").html("<span class='alert alert-danger d-block w-100 m-0 mb-2 py-2 px-2'><i class='fas fa-exclamation-circle'></i> Please fill both Micro & Macroscopic fields</span>").slideDown('fast').delay(4000).slideUp('fast');
        $('#testResultForm').animate({ scrollTop: 0 }, 'fast');
    }
});


function move_to_page(num) {
    window.location.href = "/labolatory/patients/p/"+num+"/";
}
...
function patient_history(parent, hist_date) {
    ...
    if ($(".details", parent).text() == ""){
        ...
        $.ajax({
            type: 'POST',
            url: "/labolatory/patient/history/",
            data: formData,
            dataType: 'json',
            contentType: false,
            processData: false,
            ...
            success: function(response) {
                if(response.success) {
                    ...
                    for(let i = 0; i < date_history.length; i++) {
                        ...
                        <b>Test name:</b> &nbsp; <span>${hist.test_name}</span><br>
                        ...
                        <div class="mb-3"></div>
                        <b>Microscopic:</b> &nbsp; <span>${hist.micro.replace(/\n/g, '<br>')}</span><br>
                        <b>Macroscopic:</b> &nbsp; <span>${hist.macro.replace(/\n/g, '<br>')}</span>
                        <div class="mb-2"></div>
                        ...
                        for (let y = 0; y < report_fields.length; y++) {
                            ...
                            html += `${y+1}. ${row.name}: &nbsp; <span>${row.value}</span> &nbsp; Normal range: &nbsp; <span>${row.range}</span>`;
                            if(report_fields.length > y+1) html += `<br>`;
                        }
                        
                        html += `<div class="mb-2"></div>
                        <div class="mb-2"></div>
                        <div class="border-bottom border-bblue text-bblue p-1">Reporter's summary</div><span>${hist.report.replace(/\n/g, '<br>')}</span>`;
