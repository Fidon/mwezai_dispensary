...
$(function () {
    ...
    $("#meds_inventory_table thead tr").clone(true).attr('class','filters').appendTo('#meds_inventory_table thead');
    var table = $("#meds_inventory_table").DataTable({
        fixedHeader: true,
        processing: true,
        serverSide: true,
        ajax: {
        ...
        },
        {
            targets: 8,
            createdCell: function (cell, cellData, rowData, rowIndex, colIndex) {
                var cell_content =`<a href="/pharmacy/inventory/m/${rowData.id}/" class="btn btn-bblue text-white">view</a>`;
        ...
        buttons: [
            { // Copy button
                ...
                titleAttr: "Copy",
                title: "Pharmacy inventory - Mwezai dispensary",
                exportOptions: {
...
document.addEventListener('click', e => {
    ...
    if (clickedElement.is('#hide_med_btn')) {
        e.preventDefault();
        var formData = new FormData();
        formData.append("id", $("#up_medicine_id").val());
